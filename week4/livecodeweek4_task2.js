...
function evaluateSlotMachine(arr) {
  ...
  if (status == 1) {
    result = (sort[0]+sort[1]+sort[2]) * 100;
    return "YOU WIN "+result+" Dollar, YOUR CREDIT BALANCE IS "+ (creditBalance+result) + " DOLLAR"
  } else if (status == 2) {
    result = (sort[0]+sort[1]+sort[2]) * 200;
    return "YOU WIN "+result+" Dollar, YOUR CREDIT BALANCE IS "+ (creditBalance+result) + " DOLLAR"
  } else {
    result = (sort[0]+sort[1]+sort[2]) * 50;
    return "YOU LOSE "+result+" Dollar, YOUR CREDIT BALANCE IS "+ (creditBalance-result) + " DOLLAR"
  }
